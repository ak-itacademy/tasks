...
def calculate_32_zero_bits(value: int) -> int:
    count = 0
    mask = 1  # Маска для проверки каждого бита
    for _ in range(32):
        if value & mask == 0:
            count += 1
        mask <<= 1  # Сдвигаем маску влево на 1 позицию
    return count
#
# # 2 вариант - Посчитать количество бит в 32-х битном числе, установленных в ноль.
# #def calculate_32_zero_bits(value: int) -> int:
# def calculate_32_zero_bits(value: int) -> int:
#     value = 16
#     binary_str = format(value, '032b')  # Преобразуем число в 32-битную двоичную строку
#     count = binary_str.count('0')  # Считаем количество нулевых битов
#     return count
...
def unpack_4_4(num: int) -> Tuple[int, int]:
    return num & 0b00001111, num & 0b11110000 >> 4
...
if __name__ == '__main__':
    print(calculate_32_zero_bits(16))